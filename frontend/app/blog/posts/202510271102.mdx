...
Agent Lightning 是一個用於訓練 AI agents 的強大工具，旨在以近乎零程式碼修改的方式，將您的 AI agents 轉變為可最佳化的實體。它支援任何 agent 框架，並擁抱 Reinforcement Learning 等多種先進演演算法，為 AI agents 的持續改進提供清晰路徑。
...
- 🤗 **整合多種先進演演算法**: 內建並支援 Reinforcement Learning (RL), Automatic Prompt Optimization, Supervised Fine-tuning 等多種演演算法。
...
- **演演算法驅動的改進**: 您選擇或編寫的演演算法從 `LightningStore` 讀取 spans 進行學習，並發布更新的資源，例如最佳化的 prompt templates 或新的 policy weights。
